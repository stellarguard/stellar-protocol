...
An HTTP `POST` to `<MULTISIG_ENDPOINT>` using ContentType `application/json`.
...
- **statusHref** - Required. A fully qualified url where an agent can poll for updates on the status of the multisig request. This is specified in the Status Endpoint section.
...
An HTTP `GET` to `<MULTISIG_ENDPOINT>/<id>`.

Request Parameters:

`<id>` - The id used to look up the status, which was returned in the submit transaction response.
...
- **uri** - Required. A SEP-0007 `tx` URI that represents the current state of the transaction, including any additional signatures that have been added to the transaction.
