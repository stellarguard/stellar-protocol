...
- **_links.status.href** - Required. A fully qualified url where an agent can poll for updates on the status of the multisig request. This is specified in the Status Endpoint section.
...
An HTTP `GET` to `<MULTISIG_ENDPOINT>/<id>/status`.
...
- **signers** - Required. An array of public keys that have signed the transaction.
